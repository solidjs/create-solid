...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// create-solid is installed globally on people's computers. This means
// that it is extremely difficult to have them upgrade the version and
// because there's only one global version installed, it is very prone to
// breaking changes.
//
// The only job of create-solid is to init the repository and then
// forward all the commands to the local version of create-solid.
//
// If you need to add a new command, please add it to the scripts/ folder.
//
// The only reason to modify this file is to add more warnings and
// troubleshooting information for the `create-solid` command.
//
// Do not make breaking changes! We absolutely don't want to have to
// tell people to update their global version of create-solid.
//
...
const program = new commander.Command(packageJson.name)
  ...
  .usage(`${chalk.green('<template> <project-directory>')} [options]`)
  ...
  .on('--help', () => {
    ...
    console.log(
      `      - a local path relative to the current working directory: ${chalk.green(
        'file:../my-solid-scripts'
      )}`
    );
    console.log(
      `      - a .tgz archive: ${chalk.green(
        'https://mysite.com/my-solid-scripts-0.8.2.tgz'
      )}`
    );
    console.log(
      `      - a .tar.gz archive: ${chalk.green(
        'https://mysite.com/my-solid-scripts-0.8.2.tar.gz'
      )}`
    );
    console.log(
    ...
    console.log(
      `      ${chalk.cyan(
        'https://github.com/facebook/create-solid/issues/new'
      )}`
    );
    console.log();
  })
...
function createApp(
  ...
  console.log(`Creating a new Solid app in ${chalk.green(root)}.`);
  console.log();
...
function install(root, useYarn, dependencies, verbose, isOnline) {
  return new Promise((resolve, reject) => {
    ...
    if (useYarn) {
      ...
      // https://github.com/facebook/create-solid/issues/3326.
    ...
    getPackageName(packageToInstall)
      ...
      .then(async packageName => {
        ...
        await executeNodeScript(
          ...
          [root, template, appName, verbose, originalDirectory],
          `
...
function getInstallPackage(version, originalDirectory) {
  let packageToInstall = 'solid-scripts';
...
function getPackageName(installPackage) {
  if (installPackage.match(/^.+\.(tgz|tar\.gz)$/)) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. solid-scripts-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/solid-scripts.git
    // git+ssh://github.com/mycompany/solid-scripts.git#v1.2.3
    ...
    console.error(
      chalk.red(
        'You are running Node %s.\n' +
          'Create Solid App requires Node %s or higher. \n' +
...
// https://github.com/facebook/create-solid/pull/368#issuecomment-243446094
